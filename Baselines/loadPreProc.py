...
def load_data(filename, data_path,data_path_train_filename, data_path_test_filename, save_path, test_ratio, valid_ratio, rand_state, lang, prob_type,filename_map_list, test_mode):
  ...
  data_dict_filename = ("%sraw_data~%s~%s~%s~%s~%s~%s~%s.pickle" % (save_path, filename[:-4], test_ratio, valid_ratio, rand_state, lang, prob_type,test_mode))
  if os.path.isfile(data_dict_filename):
    print("loading input data")
    with open(data_dict_filename, 'rb') as f_data:
        data_dict = pickle.load(f_data)
      ...
      data_dict = {}
      with open(data_path_train_filename, 'rb') as f_trdata:
            data_dict_train = pickle.load(f_trdata)
      with open(data_path_test_filename, 'rb') as f_tedata:
            data_dict_test = pickle.load(f_tedata)
      print (len(data_dict_train['task_1_labels']))
      print (len(data_dict_test['task_1_labels']))
      task_2_labels =[]
      task_1_labels =[]
      text_sen=[]
      data_dict['task_1_orig_labels'] = data_dict_train['task_1_labels'] + data_dict_test['task_1_labels']
      data_dict['task_2_orig_labels'] = data_dict_train['task_2_labels'] + data_dict_test['task_2_labels']
      data_dict['simple_cleaning_sent_texts'] = data_dict_train['simple_cleaning_sent_texts'] + data_dict_test['simple_cleaning_sent_texts']
      data_dict['text'] = data_dict_train['simple_cleaning_texts'] + data_dict_test['simple_cleaning_texts']
      data_dict['test_case'] = data_dict_train['text_case'] + data_dict_test['test_case']
      data_dict['source'] = data_dict_train['source'] + data_dict_test['source']
      data_dict['language'] = data_dict_train['language'] + data_dict_test['language']
      data_dict['ID'] = data_dict_train['ID'] + data_dict_test['ID']
      data_dict['tweets'] = data_dict_train['tweets'] + data_dict_test['tweets']
      data_dict['tokenized_tweets'] = data_dict_train['tokenized_tweets'] + data_dict_test['tokenized_tweets']
      data_dict['hashtags'] = data_dict_train['hashtags'] + data_dict_test['hashtags']
      data_dict['smileys'] = data_dict_train['smileys'] + data_dict_test['smileys']
      data_dict['emojis'] = data_dict_train['emojis'] + data_dict_test['emojis']
      data_dict['urls'] = data_dict_train['urls'] + data_dict_test['urls']
      data_dict['mentions'] = data_dict_train['mentions'] + data_dict_test['mentions']
      data_dict['numbers'] = data_dict_train['numbers'] + data_dict_test['numbers']
      data_dict['reserveds'] = data_dict_train['reserveds'] + data_dict_test['reserveds']
      data_dict['tweetproc_clean_texts'] = data_dict_train['tweetproc_clean_texts'] + data_dict_test['tweetproc_clean_texts']
      data_dict['emoji_texts'] = data_dict_train['emoji_texts'] + data_dict_test['emoji_texts']
      data_dict['segmented_hashtags'] = data_dict_train['segmented_hashtags'] + data_dict_test['segmented_hashtags']
      data_dict['max_num_sent'] = 13
      data_dict['max_words_sent'] = 35

      for i in range(len(data_dict['text'])):
          task_1_labels.append(conf_map['LABEL_MAP'][data_dict['task_1_orig_labels'][i]])
          task_2_labels.append(conf_map1['LABEL_MAP'][data_dict['task_2_orig_labels'][i]])
          se_list = []
          for se in sent_tokenize(data_dict['text'][i]):
            se_cl = r_white.sub(' ', r_anum.sub('', str(se).lower())).strip()
            if se_cl == "":
              continue
            words = se_cl.split(' ')
            while len(words) > data_dict['max_words_sent']:
              se_list.append(' '.join(words[:data_dict['max_words_sent']]))
              words = words[data_dict['max_words_sent']:]
            se_list.append(' '.join(words))
          text_sen.append(se_list)
      data_dict['task_1_labels'] = task_1_labels
      data_dict['task_2_labels'] = task_2_labels
      data_dict['text_sen'] = text_sen

      print (len(data_dict['task_1_labels']))
      val_index = 6977
      data_dict['max_post_length'] = max([len(post.split(' ')) for post in data_dict['text'][:val_index]])
      if test_mode:
          data_dict['train_en_ind'] = val_index
          data_dict['test_en_ind'] = len(data_dict['text'])
      else:
          data_dict['train_en_ind'] = train_index
          data_dict['test_en_ind'] = val_index
      data_dict['test_st_ind'] = data_dict['train_en_ind']

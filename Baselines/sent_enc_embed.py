...
def sent_enc_featurize(sent_enc_feats_raw, model_type, data_dict, poss_sent_enc_feats_emb_dict, use_saved_sent_enc_feats, save_sent_enc_feats, data_fold_path, save_fold_path, test_mode,bert_path,bert_max_seq_len,tbert_paths):
	...
	for sent_enc_feat_raw_dict in sent_enc_feats_raw:
		...
		s_filename = ("%ssent_enc_feat~%s.h5" % (save_fold_path, feat_name))
		if use_saved_sent_enc_feats and os.path.isfile(s_filename):
			...
			if feat_name.startswith('trainable'):
				feats = bert_tune_embed_posts(bert_path,bert_max_seq_len,data_dict['text'],feat_name,tbert_paths)
			elif feat_name == "tbert":
				feats = bert_flat_embed_posts(data_dict['text'], poss_sent_enc_feats_emb_dict[feat_name])
